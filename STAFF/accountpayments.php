...
<div class="container">
	<div style="padding: 6px 12px; border: 1px solid #ccc;">
		<h3>STAFF DASHBOARD FOR ACCOUNT REGISTRATION PAYMENTS</h3> 
		<p> Registration payments made by Kentour members</p>  
		<p>QUICK LINKS: 
		<a href="#pendingpayments"><button type="button" class="btn btn-outline-secondary">Pending</button></a>
		<a href="#approvedpayments"><button type="button" class="btn btn-outline-secondary">Approved payments</button></a>
		<a href="#rejectedpayments"><button type="button" class="btn btn-outline-secondary">Rejected Payments</button></a>
    ...
    <div style="padding: 6px 12px; border: 1px solid #ccc;height:auto; verflow: auto;" id="pendingpayments">
		...
		<table class="table table-bordered table-striped">
			...
			<tbody>
				...
				$sql = "SELECT * FROM registrationpayments WHERE status='PENDING'  ";
				...
				{	
					...
					echo '<tr>';
						...
						echo '<td>'.$row[7].'</td> '; //land id
                        echo '<td>'.$row[4].' @ '.$row[5].'</td>'; // DATE &  TIME
                        echo '<td>'.$row[6].'</td> '; //Status
                        echo '<td><a href="approveaccountpay.php?id=' . $row[0] . '"><button class="btn btn-success">APPROVE</button></a> 
                                    <a href="rejectaccountpay.php?id=' . $row[0] . '"><button class="btn btn-danger">REJECT</button></a></td>';
					echo '</tr>';
				}
				?>
			</tbody>
    ...
    <div style="padding: 6px 12px; border: 1px solid #ccc;height:auto; verflow: auto;" id="approvedpayments">
		...
		<table class="table table-bordered table-striped">
			...
			<tbody>
				...
				$sql = "SELECT * FROM registrationpayments WHERE status='APPROVED'  ";
				$result = mysqli_query($conn, $sql);
				while($row = mysqli_fetch_array($result, MYSQLI_NUM))
				{	
    ...
    <div style="padding: 6px 12px; border: 1px solid #ccc;height:auto; verflow: auto;" id="rejectedpayments">
		...
		<table class="table table-bordered table-striped">
			...
			<tbody>
				...
				$sql = "SELECT * FROM registrationpayments WHERE status='REJECTED'  ";
				$result = mysqli_query($conn, $sql);
				while($row = mysqli_fetch_array($result, MYSQLI_NUM))
				{	
