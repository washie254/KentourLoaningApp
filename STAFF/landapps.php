...
<div class="container">
	<div style="padding: 6px 12px; border: 1px solid #ccc;">
		Quick Links: 
		<a href="#pending"><button type="button" class="btn btn-outline-secondary">Pending Applications</button></a>
		<a href="#approved"><button type="button" class="btn btn-outline-secondary">Approved applications</button></a>
		<a href="#rejected"><button type="button" class="btn btn-outline-secondary">Rejected applications</button></a>
	</div>
</div>
<br>
    ...
    <div style="padding: 6px 12px; border: 1px solid #ccc;height:auto; verflow: auto;">
		...
		<table class="table table-bordered">
			...
			<tbody>
				...
				$sql = "SELECT * FROM landapplications WHERE status='PENDING'";
				...
				{	
					...
					echo '<tr>';
						...
						echo '<td>'.$row[3].'</td> '; //Amount
						echo '<td>'.$row[4].'</td> '; //DATE APPLIED
						echo '<td>'.$row[5].'</td> '; //Purpose
						echo '<td>'.$row[6].'</td> '; //Purpose
						echo '<td><a href="approveland.php?id=' . $row[0] . '"><button class="btn btn-success">APPROVE</button></a> 
						<a href="rejectlan.php?id='.$row[0].'&memb='.$row[4].'"><strong><button type="button" class="btn btn-danger">REJECT</button></td>';
					echo '</tr>';
				}
				?>
			</tbody>
...
<div class="container" id="approved">
    <div style="padding: 6px 12px; border: 1px solid #ccc;height:auto; verflow: auto;">
        <h3>APPROVED LAND APPLICATIONS</h3> 
		<p> the following are the currently approved land applications loans</p> 
		...
		<table class="table table-bordered">
			...
			<tbody>
				...
				$sql = "SELECT * FROM landapplications WHERE status='APPROVED'";
				$result = mysqli_query($conn, $sql);
				while($row = mysqli_fetch_array($result, MYSQLI_NUM))
				{	
...
<div class="container" id="rejected">
    <div style="padding: 6px 12px; border: 1px solid #ccc;height:auto; verflow: auto;">
        <h3>REJECTED LAND APPLICATIONS</h3> 
		<p> the following are the currently rejected land applications loans</p> 
		...
		<table class="table table-bordered">
			...
			<tbody>
				...
				$sql = "SELECT * FROM landapplications WHERE status='REJECTED'";
				$result = mysqli_query($conn, $sql);
				while($row = mysqli_fetch_array($result, MYSQLI_NUM))
				{	
