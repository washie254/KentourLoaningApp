    ...
    <div style="padding: 6px 12px; border: 1px solid #ccc;height:auto; verflow: auto;" id="pendingpayments">
		...
		<table class="table table-bordered table-striped">
			...
			<tbody>
				...
				$sql = "SELECT * FROM cashloanpayments WHERE status='PENDING'  ";
				...
				{	
					...
					echo '<tr>';
                        ...
                        echo '<td><a href="approvecash.php?id=' . $row[0] . '"><button class="btn btn-success">APPROVE</button></a> 
                                    <a href="rejectcash.php?id=' . $row[0] . '"><button class="btn btn-danger">REJECT</button></a></td>';
					echo '</tr>';
				}
				?>
			</tbody>
    ...
    <div style="padding: 6px 12px; border: 1px solid #ccc;height:auto; verflow: auto;" id="approvedpayments">
		...
		<table class="table table-bordered table-striped">
			...
			<tbody>
				...
				$sql = "SELECT * FROM cashloanpayments WHERE status='APPROVED'  ";
				$result = mysqli_query($conn, $sql);
				while($row = mysqli_fetch_array($result, MYSQLI_NUM))
				{	
    ...
    <div style="padding: 6px 12px; border: 1px solid #ccc;height:auto; verflow: auto;" id="rejectedpayments">
		...
		<table class="table table-bordered table-striped">
			...
			<tbody>
				...
				$sql = "SELECT * FROM cashloanpayments WHERE status='REJECTED'  ";
				$result = mysqli_query($conn, $sql);
				while($row = mysqli_fetch_array($result, MYSQLI_NUM))
				{	
