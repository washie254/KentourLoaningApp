						 ...
						 <div style="padding: 6px 12px; border: 1px solid #ccc;height:auto; verflow: auto;">
                            <form action="payloans.php" method="post">
                                <?php include('errors.php'); ?>
                                    ...
                                    <label for="amount">Select Loans</label>
                                    <?php
									  require('connect-db.php');
									  $user = $_SESSION['username'];
	  
									  $sql = "SELECT * FROM cashloans WHERE status='APPROVED' AND membername='$user'";
									  $result = mysqli_query($conn, $sql);
									  echo '<div><select name="loanID" class="form-group">';
									  while($row = mysqli_fetch_array($result, MYSQLI_NUM))
									  {	
										echo '<option value='.$row[0].'>'.$row[1].'</option> '; // l ID 
									  }
									  echo '</select></div>';
								  ?>
                                    </div>
                            ...
                            <input name="mid" value="<?=$userid?>" style="opacity: 0;" />
                                    <div class="form-group">
                                        <button type="submit"  class="btn btn-success" name="repay_loan">PAY LOAN</button>
                                    </div>
